...
class GuideTypes(object):
    ...
    filePath = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvsupertugaguide', 'guides.ini'))
    ...
    def __init__(self):
        try:
            ...
            xbmc.log('[script.tvsupertugaguide] GuideTypes collected: %s' % str(self.guideTypes), xbmc.LOGDEBUG)
    ...
    def SectionMap(self, section):
        ...
        for option in options:
            try:
                ...
                if dict1[option] == -1:
                    xbmc.log('[script.tvsupertugaguide] skip: %s' % option, xbmc.LOGDEBUG)
            except:
    ...
    def getGuideDataItem(self, id, item):
        ...
        try:
            ...
            xbmc.log('[script.tvsupertugaguide] DataItem with index %s not found' % item)
        return value


    def getGuideById(self, id):
        xbmc.log('[script.tvsupertugaguide] Finding Guide with ID: %s' % id)
        ret = []
        for guide in self.guideTypes:
            if guide[self.GUIDE_ID] == int(id):
                ret = guide
                xbmc.log('[script.tvsupertugaguide] Found Guide with data: %s' % str(guide))
        return ret
...
if __name__ == '__main__':
    ...
    if ret >= 0:
        ...
        if xbmc.getCondVisibility('system.platform.android') and int(ver) < 15:
            ...
            xbmc.log('[script.tvsupertugaguide] Running on ANDROID with Kodi v%s --> using workaround!' % str(ver), xbmc.LOGDEBUG)
