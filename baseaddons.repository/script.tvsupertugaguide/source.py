...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.tvsupertugaguide] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.tvsupertugaguide] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.tvsupertugaguide] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.tvsupertugaguide] Updating caches...', xbmc.LOGDEBUG)
    ...
    def _getEPGView(self, channelStart, date, progress_callback, clearExistingProgramList,category):
        ...
        if category and category != "All Channels":
            f = xbmcvfs.File('special://profile/addon_data/script.tvsupertugaguide/categories.ini','rb')
...
class XMLTVSource(Source):
    PLUGIN_DATA = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvsupertugaguide'))
    KEY = 'xmltv'
    INI_TYPE_IVUE = 0
    INI_TYPE_CUSTOM = 1
    INI_FILE = 'addons.ini'
    ...
    CATEGORIES_INI_FILE = 'categories.ini'
...
#Karls custom xml import for creator
        ...
        if self.xmltvType == gType.CUSTOM_FILE_ID:
            xmltvTemp = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvsupertugaguide', 'custom.xml'))
	...
	catsFile = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvsupertugaguide', 'categories.ini'))
	ivueFile = 'https://raw.githubusercontent.com/programacaotv/programacaotv/master/guide/categories.ini'

#Karl has Added iVue Categories

        if addon.getSetting('ivue-categories.ini.enabled') == 'true':
            urllib.urlretrieve(ivueFile, catsFile)
            addon.setSetting('ivue-categories.ini.enabled', 'false')
#Karl has Added custom categories
...
#End of Karls changes
    ...
    def parseXMLTVDate(self, origDateString):
        if origDateString.find(' ') != -1:
            ...
            xbmc.log('[script.tvsupertugaguide] Date to normalize: ' + dateString, xbmc.LOGDEBUG)
            try:
            ...
            t = t + td_local
            xbmc.log('[script.tvsupertugaguide] Import Time adjusted from: ' + str(t_tmp) + ' to: ' + str(t), xbmc.LOGDEBUG)
            return t
            ...
            if event == "end":
                ...
                if elem.tag == "programme":
                    ...
                    if iconElement is not None:
                        icon = iconElement.get("src").replace(' ', '%20')  # needed due to fetching from a server!
                    if not description:
