...
try:
    ADDON = xbmcaddon.Addon('script.tvsupertugaguide')
    if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
        Service()
    if ADDON.getSetting('autostart') == "true":
        xbmc.executebuiltin("RunAddon(script.tvsupertugaguide)")
except source.SourceNotConfiguredException:
    pass  # ignore
except Exception, ex:
    xbmc.log('[script.tvsupertugaguide] Uncaught exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)