...
class StreamsService(object):
    def __init__(self, addon):
        ...
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvsupertugaguide', 'addons.ini'))
            xbmc.log('[script.tvsupertugaguide] addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.tvsupertugaguide', customFile.split('/')[-1]))
            xbmc.log('[script.tvsupertugaguide] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)
    ...
    def detectStream(self, channel):
        ...
        for id in self.getAddons():
            ...
            for (label, stream) in self.getAddonStreams(id):
                ...
                if label.lower() == channel.title.lower() or label.lower() == channel.title.lower() + " (link 1)" or label.lower() == channel.title.lower() + " (link 2)" or label.lower() == channel.title.lower() + " (link 3)" or label.lower() == channel.title.lower() + " (sportsdevil)" or label.lower() == channel.title.lower() + " (oficial)" or label.lower() == channel.title.lower() + " (android ace)" or label.lower() == channel.title.lower() + " (vpn)" or label.lower() == channel.title.lower() + " (off)":
