...
ADDONID = 'script.tvsupertugaguide'
...
class TVGuide(xbmcgui.WindowXML):
	...
	def onActionEPGMode(self, action):
		...
		if action.getId() == ACTION_LEFT:
			...
			xbmc.log('[script.tvsupertugaguide] Unhandled ActionId: '
	...
	def _showExitMenu(self):
		...
		resp = dialog.select('[COLOR fffea800]TVSUPERTUGA Menu[/COLOR]', ['Categories', 'Exit Guide', 'Change guide type', 'Update Channels', 'Soft Reset']) 
		if resp == 0:
			self._showCatMenu()         
		if resp == 1:
			self.close()
		if resp == 2:
			xbmc.executebuiltin('RunScript(%s)' % (os.path.join(HOME, 'guideTypes.py')))
			#import Resetaddonini
...
class CatMenu(xbmcgui.WindowXMLDialog):
	...
	def __new__(
		...
		return super(CatMenu, cls).__new__(cls,
				'script-tvguide-categories.xml', ADDON.getAddonInfo('path'), SKIN)
	...
	def onAction(self, action):
		if action.getId() in [KEY_CONTEXT_MENU]:
			...
			if kodi < 16:
				...
				f = xbmcvfs.File('special://profile/addon_data/script.tvsupertugaguide/categories.ini','wb')
		...
		for id in self.streamingService.getAddons():
			try:
				...
				item = xbmcgui.ListItem(addon.getAddonInfo('name'),
						iconImage='special://profile/addon_data/script.tvsupertugaguide/categories.ini')
		...
		for (id, label, url) in self.addons:
			try:
				...
				if id == 'kodi-favourite':
					...
					name = ''
				elif id == 'script.tvsupertugaguide':
