		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
					...
					downloads = "  [COLOR lawngreen] [/COLOR][COLOR cyan] [/COLOR] [COLOR red]" + str(wiz.count_total(name)) + "[/COLOR]"
					addDir('[%s] %s (v%s) %s' % (float(kodi), name, version, downloads), 'viewbuild', name, description=description, fanart=fanart,icon=icon, menu=menu, themeit=THEME2)
			else:
				if count18 > 0:
					state = '+' if SHOW18 == 'false' else '-'
					addFile('[B]%s [COLOR lime]LEIA BUILDS[/COLOR](%s)[/B]' % (state, count18), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW18 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count17 > 0:
					...
					addFile('[B]%s [COLOR lime]KRYPTON BUILDS[/COLOR](%s)[/B]' % (state, count17), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW17 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count16 > 0:
					...
					addFile('[B]%s [COLOR lime]JARVIS BUILDS[/COLOR](%s)[/B]' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
					if SHOW16 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
					...
					if plugin.lower() == 'section':
						...
						try:
							...
							if os.path.exists(add):
								name   = "[COLOR lime][Installed][/COLOR] %s" % name
						except:
							pass
						x += 1
				...
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						...
						if selected == 0:
							...
							if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove the addon_data for:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR cyan]Yes Remove[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						...
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to install the repository for [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR cyan]Yes Install[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"): 
...
def maintMenu(view=None):
	on = '[B][COLOR cyan]ON[/COLOR][/B]'; off = '[B][COLOR red]OFF[/COLOR][/B]'
	...
	wizlogsize = ': [COLOR red]Not Found[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR cyan]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
	if includeall == 'true':
		includebob = 'true'
		includepho = 'true'
		includespe = 'true'
	...
	addDir ('[COLOR cyan][B]CLEANING TOOLS[/B][/COLOR]'       ,'maint', 'clean',  icon=ICONMAINT, themeit=THEME1)
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('Total Clean Up: [COLOR cyan][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize)  ,'fullclean',       icon=ICONMAINT, themeit=THEME3)
		addFile('Clear Cache: [COLOR cyan][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache)     ,'clearcache',      icon=ICONMAINT, themeit=THEME3)
		addFile('Clear Packages: [COLOR cyan][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack)   ,'clearpackages',   icon=ICONMAINT, themeit=THEME3)
		addFile('Clear Thumbnails: [COLOR cyan][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),'clearthumb',      icon=ICONMAINT, themeit=THEME3)
	...
	addDir ('[COLOR cyan][B]ADDON TOOLS[/B][/COLOR]',       'maint', 'addon',  icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('[COLOR cyan][B]MISC MAINTENANCE[/B][/COLOR]'     ,'maint', 'misc',   icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('[COLOR cyan][B]BACK UP/RESTORE[/B][/COLOR]'     ,'maint', 'backup',   icon=ICONMAINT, themeit=THEME1)
	...
	addDir ('[COLOR cyan][B]SYSTEM TWEAKS/FIXES[/B][/COLOR]',       'maint', 'tweaks', icon=ICONMAINT, themeit=THEME1)
	if view == "tweaks" or SHOWMAINT == 'true': 
		if not ADVANCEDFILE == 'http://' and not ADVANCEDFILE == '':
			
			addDir('[COLOR cyan]ADVANCED SETTINGS[/COLOR]',            'advancedsetting',  icon=ICONMAINT, themeit=THEME3)
		else: 
	...
	addFile('[COLOR lime][B]SHOW ALL MAINTENANCE:[/B][/COLOR] %s' % maint.replace('true',on).replace('false',off) ,'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME2)
	addDir ('[COLOR yellow][B]<< RETURN TO MAIN MENU[/B][/COLOR]', icon=ICONMAINT, themeit=THEME2)
	addFile('[COLOR lime][B]THIRD PARTY WIZARDS:[/B][/COLOR] %s' % thirdparty.replace('true',on).replace('false',off) ,'togglesetting', 'enable3rd', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	if thirdparty == 'true':
	...
	addFile('[COLOR cyan][B]AUTO CLEAN[/B][/COLOR]', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	addFile('[COLOR lime][B]--- AUTO CLEAN FEQUENCY:[/B][/COLOR] %s' % autoclean.replace('true',on).replace('false',off) ,'togglesetting', 'autoclean',   icon=ICONMAINT, themeit=THEME3)
	if autoclean == 'true':
		addFile('[COLOR lime][B]--- AUTO CLEAN FEQUENCY:[/B][/COLOR] [B][COLOR cyan]%s[/COLOR][/B]' % feq[AUTOFEQ], 'changefeq', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- CLEAR CACHE ON STARTUP:[/B][/COLOR] %s' % cache.replace('true',on).replace('false',off), 'togglesetting', 'clearcache', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- CLEAR PACKAGES ON STARTUP:[/B][/COLOR] %s' % packages.replace('true',on).replace('false',off), 'togglesetting', 'clearpackages', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- CLEAR OLD THUMBS ON STARTUP:[/B][/COLOR] %s' % thumbs.replace('true',on).replace('false',off), 'togglesetting', 'clearthumbs', icon=ICONMAINT, themeit=THEME3)
	addFile('[COLOR cyan][B]CLEAR VIDEO CACHE[/B][/COLOR]', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	addFile('[COLOR lime][B]INCLUDE VIDEO CACHE IN CLEAR CACHE:[/B][/COLOR] %s' % includevid.replace('true',on).replace('false',off), 'togglecache', 'includevideo', icon=ICONMAINT, themeit=THEME3)
	if includevid == 'true':
		addFile('[COLOR lime][B]--- INCLUDE ALL VIDEO ADDONS:[/B][/COLOR] %s' % includeall.replace('true',on).replace('false',off), 'togglecache', 'includeall', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE BOB:[/B][/COLOR] %s' % includebob.replace('true',on).replace('false',off), 'togglecache', 'includebob', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE PHOENIX:[/B][/COLOR] %s' % includepho.replace('true',on).replace('false',off), 'togglecache', 'includephoenix', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE SPECTO:[/B][/COLOR] %s' % includespe.replace('true',on).replace('false',off), 'togglecache', 'includespecto', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE EXODUS:[/B][/COLOR] %s' % includeexo.replace('true',on).replace('false',off), 'togglecache', 'includeexodus', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE SALTS:[/B][/COLOR] %s' % includesal.replace('true',on).replace('false',off), 'togglecache', 'includesalts', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE SALTS HD LITE:[/B][/COLOR] %s' % includeshd.replace('true',on).replace('false',off), 'togglecache', 'includesaltslite', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE ONE CHANNEL:[/B][/COLOR] %s' % includeone.replace('true',on).replace('false',off), 'togglecache', 'includeonechan', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- INCLUDE GENESIS:[/B][/COLOR] %s' % includegen.replace('true',on).replace('false',off), 'togglecache', 'includegenesis', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR lime][B]--- ENABLE ALL VIDEO ADDONS[/B][/COLOR]', 'togglecache', 'true', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR red]--- DISABLE ALL VIDEO ADDONS', 'TOGGLECACHE[/B][/COLOR]', 'false', icon=ICONMAINT, themeit=THEME3)
	setView('files', 'viewType')
...
def writeAdvanced(name, url):
	ADVANCEDWORKING = wiz.workingURL(url)
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to overwrite your current Advanced Settings with [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR cyan]Overwrite[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to download and install [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR cyan]Install[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
...
def saveMenu():
	on = '[COLOR cyan]ON[/COLOR]'; off = '[COLOR red]OFF[/COLOR]'
...
def traktMenu():
	trakt = '[COLOR cyan]ON[/COLOR]' if KEEPTRAKT == 'true' else '[COLOR red]OFF[/COLOR]'
	...
	for trakt in traktit.ORDER:
		...
		else:                        addFile('[COLOR cyan]Addon Data: %s[/COLOR]' % auser,'authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		if user == "":
		...
		else:                        addFile('[COLOR cyan]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
...
def realMenu():
	real = '[COLOR cyan]ON[/COLOR]' if KEEPREAL == 'true' else '[COLOR red]OFF[/COLOR]'
	...
	for debrid in debridit.ORDER:
		...
		else:                        addFile('[COLOR cyan]Addon Data: %s[/COLOR]' % auser,'authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
		if user == "":
...
def loginMenu():
	login = '[COLOR cyan]ON[/COLOR]' if KEEPLOGIN == 'true' else '[COLOR red]OFF[/COLOR]'
	...
	for login in loginit.ORDER:
		...
		else:                        addFile('[COLOR cyan]Addon Data: %s[/COLOR]' % auser,'authlogin', login, icon=icon, fanart=fanart, menu=menu)
		if user == "":
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to [COLOR %s]Force close[/COLOR] kodi or [COLOR %s]Reload Profile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR cyan]Reload Profile[/COLOR][/B]", nolabel="[B][COLOR red]Force Close[/COLOR][/B]")
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			replace = {'audio.':'[COLOR orange][AUDIO] [/COLOR]', 'metadata.':'[COLOR cyan][METADATA] [/COLOR]', 'module.':'[COLOR orange][MODULE] [/COLOR]', 'plugin.':'[COLOR blue][PLUGIN] [/COLOR]', 'program.':'[COLOR orange][PROGRAM] [/COLOR]', 'repository.':'[COLOR gold][REPO] [/COLOR]', 'script.':'[COLOR cyan][SCRIPT] [/COLOR]', 'service.':'[COLOR cyan][SERVICE] [/COLOR]', 'skin.':'[COLOR dodgerblue][SKIN] [/COLOR]', 'video.':'[COLOR orange][VIDEO] [/COLOR]', 'weather.':'[COLOR yellow][WEATHER] [/COLOR]'}
			for rep in replace:
				folderdisplay = folderdisplay.replace(rep, replace[rep])
			if foldername in EXCLUDES: folderdisplay = '[COLOR cyan][B][PROTECTED][/B][/COLOR] %s' % folderdisplay
	...
	for folder in sorted(fold, key = lambda x: x):
		...
		if os.path.exists(addonxml):
			...
			try:
				...
				state  = "[COLOR cyan][Enabled][/COLOR]"
				goto   = "false"
			except:
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR cyan]Apply Fix[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]WARNING!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build is not currently installed." % (COLOR2, COLOR1, name), "Would you like to apply the guiFix anyways?.[/COLOR]", nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR cyan]Apply Fix[/COLOR][/B]')
		if yes:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Gui fix has been installed.  Would you like to Reload the profile or Force Close Kodi?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Reload Profile[/COLOR][/B]", nolabel="[B][COLOR cyan]Force Close[/COLOR][/B]")
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to [COLOR %s]Force close[/COLOR] kodi or [COLOR %s]Reload Profile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Reload Profile[/COLOR][/B]", nolabel="[B][COLOR cyan]Force Close[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Build [COLOR %s]%s[/COLOR] comes with [COLOR %s]%s[/COLOR] different themes" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Would you like to install one now?[/COLOR]", yeslabel="[B][COLOR cyan]Install Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]"):
		...
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to install the theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'for [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR cyan]Install Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]")
		if installtheme:
			...
			if url not in ["fresh", "normal"]:
				...
				if test == True:
					...
					#if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Installing the theme [COLOR %s]%s[/COLOR] requires the skin to be swaped back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, theme, COLOR1, skin[5:]), "Would you like to switch the skin?[/COLOR]", yeslabel="[B][COLOR cyan]Switch Skin[/COLOR][/B]", nolabel="[B][COLOR red]Don't Switch[/COLOR][/B]"):
...
def thirdPartyInstall(name, url):
	...
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to preform a [COLOR %s]Fresh Install[/COLOR] or [COLOR %s]Normal Install[/COLOR] for:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR cyan]Fresh Install[/COLOR][/B]", nolabel="[B][COLOR red]Normal Install[/COLOR][/B]")
	if type == 1:
	...
	if int(float(percent)) > 0:
		...
		if int(float(errors)) > 0:
			yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Would you like to view the errors?[/COLOR]', nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR cyan]View Errors[/COLOR][/B]')
			if yes:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox(ADDONTITLE, error)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to download and install:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR cyan]Download[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
...
def removeAddon(addon, name, over=False):
	if not over == False:
		...
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Are you sure you want to delete the addon:'% COLOR2, 'Name: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR cyan]Remove Addon[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]')
	if yes == 1:
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to remove [COLOR %s]ALL[/COLOR] addon data stored in you Userdata folder?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR cyan]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to remove [COLOR %s]ALL[/COLOR] addon data stored in you Userdata folder for uninstalled addons?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR cyan]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
			total = 0
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to remove [COLOR %s]ALL[/COLOR] empty addon data folders in you Userdata folder?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR cyan]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
		...
		if addon in EXCLUDES:
			...
			if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you also like to remove the addon data for:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR cyan]Remove Data[/COLOR][/B]', nolabel='[B][COLOR red]Don\'t Remove[/COLOR][/B]'):
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like replace the current [COLOR %s]%s[/COLOR] file?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR cyan]Yes Replace[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"): continue
					else: os.remove(old)
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Do you wish to restore your" % COLOR2, "Kodi configuration to default settings", "Before installing the local backup?[/COLOR]", nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR cyan]Continue[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Do you wish to restore your" % COLOR2, "Kodi configuration to default settings", "Before installing [COLOR %s]%s[/COLOR]?" % (COLOR1, install), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR cyan]Continue[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Do you wish to restore your" % COLOR2, "Kodi configuration to default settings?[/COLOR]", nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR cyan]Continue[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
			#yes=DIALOG.yesno(ADDONTITLE, "[COLOR %s]The skin needs to be set back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, skin[5:]), "Before doing a fresh install to clear all Texture files,", "Would you like us to do that for you?[/COLOR]", yeslabel="[B][COLOR cyan]Switch Skins[/COLOR][/B]", nolabel="[B][COLOR red]I'll Do It[/COLOR][/B]";
			#if yes:
			skinSwitch.swapSkins(skin)
			x = 0
			xbmc.sleep(1000)
		...
		if KEEPSUPER == 'true':
			EXCLUDES.append('script.module.projectx.wizard')
			EXCLUDES.append('script.module.Community.dl')	
...
####THANKS GUYS @ NaN #######
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to clear cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR cyan]Clear Cache[/COLOR][/B]'):
		wiz.clearCache()
		#clearThumb()

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to clear cache, packages and thumbnails?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancel Process[/COLOR][/B]',yeslabel='[B][COLOR cyan]Clean All[/COLOR][/B]'):
		wiz.clearCache()
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to delete the %s and Thumbnails folder?' % (COLOR2, latest), "They will repopulate on the next startup[/COLOR]", nolabel='[B][COLOR red]Don\'t Delete[/COLOR][/B]', yeslabel='[B][COLOR cyan]Delete Thumbs[/COLOR][/B]')
	if choice == 1:
...
def emumenu():
	...
	if len(match) > 0:
		if wiz.platform() == 'android':
			...
			DIALOG.ok(ADDONTITLE, "[COLOR yellow]Execute download do RetroArch para Windows[/COLOR]", " Leia em http://tinyurl.com/RetroFTG para um tutorial(eng)")
		elif wiz.platform() == 'linux':
			DIALOG.ok(ADDONTITLE, "[COLOR yellow]Execute download do RetroArch para Linux[/COLOR]", " Leia em http://tinyurl.com/RetroFTG para um tutorial(eng)")
def rompackmenu():
...
def net_tools(view=None):
	...
	addFile ('[COLOR cyan][B]SPEED TESTER[/B][/COLOR]' ,'speed', icon=ICONSPEED, themeit=THEME1)

	addDir ('[COLOR cyan][B]VIEW IP ADDRESS & MAC ADDRESS[/B][/COLOR]',        'viewIP',    icon=ICONSPEED, themeit=THEME1)
	setView('files', 'viewType')
def viewIP():
	...
	setView('files', 'viewType')
def speedMenu():
	xbmc.executebuiltin('Runscript("special://home/addons/script.module.Community.dl/speedtest.py")')
######################################################################################################################
######################################################################################################################
params=get_params()
url=None
