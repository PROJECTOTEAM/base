...
class MyWindowFight(xbmcgui.WindowDialog):
	...
	def __init__(self,owner,bgArt,MyHand,TheirHand):
		...
		self.txtResultBGL2=xbmcgui.ControlImage(l,t,w,h,self.ow.b1,aspectRatio=0); 
		...
		for z in [self.txtResultBGL2,self.txtResultBGL,self.txtResultL]: self.addControl(z); z.setAnimations([('WindowOpen','effect=fade delay=1500 time=1000 start=0')]); 
		self.txtResultBGL2.setAnimations([('WindowOpen','effect=fade delay=1500 time=1000 start=0 end=80')]); 
		## ### ## Hands - 3 objects
		#f_hand=artp('nutshell_down_f'); 
		nf_hand=artp('nutshell_down_nf'); 
		#f_hand=artp('nutshell_up_winner'); 
		#nf_hand=artp('nutshell_up'); 
		if MyHand==TheirHand: f_hand=artp('nutshell_up_winner'); 
		else: f_hand=artp('nutshell_up'); 
		w=87; h=86; w=w*3; h=h*3; t=self.scr['H']-h; msg=""; 
		l=(self.scr['W']/3)-((w*1)+50); #msg=""+self.Hands[0]; 
		aniHands='effect=rotate time=2000 start=180 '
		self.button[1]=xbmcgui.ControlImage(l,t,w,h,nf_hand,aspectRatio=0); self.addControl(self.button[1])
		self.button[1].setAnimations([('WindowOpen',aniHands+' delay=1 center='+str(l+w)+','+str(t+h))]); 
		l=(self.scr['W']/2)-((int(w*0.5))+40); #msg=""+self.Hands[1]; 
		self.button[2]=xbmcgui.ControlImage(l,t,w,h,nf_hand,aspectRatio=0); self.addControl(self.button[2])
		self.button[2].setAnimations([('WindowOpen',aniHands+' delay=1 center='+str(l+w)+','+str(t+h))]); 
		l=(self.scr['W']/2)+((int(w*0.5))+100); #msg=""+self.Hands[2]; 
		self.button[3]=xbmcgui.ControlImage(l,t,w,h,nf_hand,aspectRatio=0); self.addControl(self.button[3])
		self.button[3].setAnimations([('WindowOpen',aniHands+' delay=1 center='+str(l+w)+','+str(t+h))]); 
		self.button[int(MyHand)].setImage(f_hand); 
		...
		if MyHand==TheirHand: tResult1='Congrats'; tResult2="You've won!"; 
		else: tResult1='Sorry'; tResult2='Better luck next time.'; 
		self.txtResultL.setLabel(""+tResult1+"[CR]"+tResult2+"")
...
class MyWindow(xbmcgui.Window):
	...
	def makePageItems(self):
		...
		self.HoroTxt.setText("Find the pea under one of the nutshells, to win the game."); 
		## ### ## Hands
	...
	def DoFight(self,MyHand):
		...
		TheirHand=self.Hands[random.randint(0,len(self.Hands)-1)]; 
		deb('MyHand',str(MyHand)); deb('TheirHand',str(TheirHand)); 
		#for z in ['3','2','1']: 
		#	self.TempWindow2=MyWindowCountDown(owner=self,bgArt=artp('hubhug_321_'+z)); 
		#	#self.TempWindow2.doModal(); 
		#	for i in range(0,100): self.TempWindow2.show(); 
		#	del self.TempWindow2; 
		self.TempWindow2=MyWindowFight(owner=self,bgArt=self.Fanart,MyHand=MyHand,TheirHand=TheirHand); 
		self.TempWindow2.doModal(); 
