...
ROMLOC           = xbmc.translatePath(os.path.join('C:\\roms\\',''))
...
def index():
	...
	if wiz.platform() == 'windows' or DEVELOPER == 'true': addDir (''       ,'retromenu', icon=ICONAPK, themeit=THEME1)

	if not ADDONFILE == 'http://': addDir ('' ,'addons', icon=ICONADDONS, themeit=THEME1)


	addDir ('' ,'net', icon=ICONSPEED, themeit=THEME1)
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
					...
					downloads = " | [COLOR cyan] Downloads:[/COLOR][COLOR cyan] Total:[/COLOR] [COLOR yellow]" + str(wiz.count_total(name)) + "[/COLOR]"
...
def apkScraper(name=""):
	if name == 'kodi':
		kodiurl1 = 'http://mirrors.kodi.tv/releases/windows/arm/'
		kodiurl2 = 'http://mirrors.kodi.tv/releases/windows/arm/old/'
	...
	#	match2 = re.compile('<tr><td><a href="(.+?)">(.+?)</a></td><td>(.+?)</td><td>(.+?)</td></tr>').findall(url2)
		...
		addFile("Official Kodi EXE\'s", themeit=THEME1)
		rc = False
		for url, name, size, date in match1:
			if url in ['../', 'old/']: continue
			if not url.endswith('.EXE'): continue
		...
		addFile("Official SPMC EXE\'s", themeit=THEME1)
...
def apkMenu(url=None):
	if url == None:
		addDir ('Official Kodi EXE\'s', 'apkscrape', 'kodi', icon=ICONAPK, themeit=THEME1)
		addDir ('Official SPMC EXE\'s', 'apkscrape', 'spmc', icon=ICONAPK, themeit=THEME1)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	if not APKFILE == 'http://':
		if url == None:
		...
		if APKWORKING == True:
			...
			if len(match) > 0:
				...
				for name, section, url, icon, fanart, adult, description in match:
					...
					if section.lower() == 'yes':
						x += 1
						addDir ("[B]%s[/B]" % name, 'EXE', url, description=description, icon=icon, fanart=fanart, themeit=THEME3)
					else:
						x += 1
			...
			else: wiz.log("[EXE Menu] ERROR: Invalid Format.", xbmc.LOGERROR)
		else: 
			wiz.log("[EXE Menu] ERROR: URL for EXE list not working.", xbmc.LOGERROR)
			addFile('Url for txt file not valid', '', themeit=THEME3)
			addFile('%s' % APKWORKING, '', themeit=THEME3)
		return
	else: wiz.log("[EXE Menu] No EXE list added.")
	setView('files', 'viewType')
		...
		if ADDONWORKING == True:
			...
			if len(match) > 0:
				...
				for name, plugin, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					...
					if x < 1:
						addFile("No home added to this menu yet!", '', themeit=THEME2)
			else: 
...
def developer():
	...
	addFile('Test EXE',             'testapk',          themeit=THEME1)
...
def apkInstaller(EXE, url):
	wiz.log(EXE)
	wiz.log(url)
	if wiz.platform() == 'windows':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to download and install:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, EXE), yeslabel="[B][COLOR green]Download[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]")
		if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: Install Cancelled[/COLOR]' % COLOR2); return
		display = EXE
		if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
		if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]EXE Installer: Invalid EXE Url![/COLOR]' % COLOR2); return
		DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, display),'', 'Please Wait')
		lib=os.path.join(PACKAGES, "%s.EXE" % EXE.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', ''))
		try: os.remove(lib)
		except: pass
		downloader.download(url, lib, DP)
		xbmc.sleep(100)
		DP.close()
		notify.apkInstaller(EXE)
		wiz.ebi('StartAndroidActivity("","windows.intent.action.VIEW","application/vnd.windows.package-archive","file:'+lib+'")')
	else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: None windows Device[/COLOR]' % COLOR2)
...
def retromenu():
	...
	if wiz.platform() == 'windows' or DEVELOPER == 'true': addDir ('[COLOR cyan]PROGRAMAS WINDOWS[/COLOR]'       ,'emumenu', icon=ICONRETRO,     themeit=THEME1)


	addDir ('[COLOR cyan]JOGOS PACK ZIPS WINDOWS[/COLOR]'       ,'rompackmenu', icon=ICONRETRO,     themeit=THEME1)
def emumenu():
...
def UNZIPROM():
	...
	if wiz.platform() == 'windows':
		...
		DIALOG.ok(ADDONTITLE, "[COLOR yellow]Download complete, ROM Location: [/COLOR][COLOR cyan]" + ROMLOC + "[/COLOR]")
	elif wiz.platform() == 'windows':
		extract.all(lib,WROMLOC,DP)
		DIALOG.ok(ADDONTITLE, "[COLOR yellow]Download complete, ROM Location: [/COLOR][COLOR cyan]" + WROMLOC + "[/COLOR]")
	elif wiz.platform() == 'linux':
		extract.all(lib,WROMLOC,DP)
		DIALOG.ok(ADDONTITLE, "[COLOR yellow]Download complete, ROM Location: [/COLOR][COLOR cyan]" + WROMLOC + "[/COLOR]")
