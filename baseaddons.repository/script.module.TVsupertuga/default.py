...
addon_id   = 'script.module.TVsupertuga'
	...
	if xbmcaddon.Addon().getSetting('enablexxxpass')=='true':
		...
		if (kb.isConfirmed()):
			...
			if pw == xbmcaddon.Addon().getSetting('xxxpass'):
				...
				xbmcgui.Dialog().ok('[COLOR red][B]TVsupertuga[/B][/COLOR]','Incorrect Password, Please Try Again')
				return
	else:
		addDir("The Best Videos",xxxurl+'/best',24,icon,fanart,'')
		addDir("Latest Videos",xxxurl,24,icon,fanart,'')
...
def setxxxpass():
	d = xbmcgui.Dialog().yesno('[COLOR red]TVsupertuga[/COLOR]','Would You Like To Set a Password for the XXX Section?')
	if d:
		...
		if (kb.isConfirmed()):
			pw = kb.getText()
			if pw =="":
				xbmcgui.Dialog().notification('[COLOR red]Password Cannot Be Blank[/COLOR]','TVsupertuga')
				setxxxpass()
			else:
				xbmcaddon.Addon().setSetting('xxxpass',pw)
				xbmcaddon.Addon().setSetting('enablexxxpass','true')
				xbmcgui.Dialog().ok('[COLOR red]TVsupertuga[/COLOR]','Password has been set')
...
def home():
	home = xbmc.executebuiltin('XBMC.RunAddon(plugin://plugin.video.TVsupertuga/?action=)')
	return home
...
def toongetresolve(name,url):
    ...
    try:
		...
		xbmcgui.Dialog().notification('[COLOR red][B]TVsupertuga[/B][/COLOR]','Oops, This Link Is Down!')
...
def radio():
	open =OPEN_URL('https://raw.githubusercontent.com/sClarkeIsBack/TVsupertuga/master/Links/RADIO.xml')
...
def showpremiumimage():
	premium_jpg = xbmc.translatePath(os.path.join('special://home/addons/script.module.TVsupertuga/resources/premium', 'premium_image.jpg'))
...
# OpenELEQ: query & type-parameter (added to line above)
	...
	xbmc.executebuiltin('XBMC.RunScript(script.module.TVsupertuga)')
	...
	xbmc.executebuiltin('Addon.OpenSettings(plugin.video.TVsupertuga)')
	sys.exit()
	...
	if xbmcaddon.Addon('plugin.video.TVsupertuga').getSetting('Username') == "":
		d = xbmcgui.Dialog().yesno('[COLOR ffff0000][B]Premium Community[/B][/COLOR]','Voce ja esta registrado para se juntar ao nosso complemento?')
		if not d:
			showpremiumimage()
			sys.exit()
		else:
			from resources.premium import premium
			d = xbmcgui.Dialog().yesno('[COLOR ffff0000][B]Premium Community[/B][/COLOR]','Otimo! Voce precisara inserir seus detalhes de login na Configuracao de complementos','Deseja que possamos abrir as configuracoes para voce agora?')
			if d:
